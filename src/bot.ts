...
async function handleMessage(ctx: Context, message: string, forceVoiceReply = false, skipLog = false): Promise<void> {
  ...
  if (decomposition.isCompound && decomposition.tasks.length > 1) {
    // Compound message: execute each task sequentially within the same
    // session so context carries forward between subtasks.
    logger.info(
      { taskCount: decomposition.tasks.length, tasks: decomposition.tasks.map((t) => t.task.slice(0, 60)) },
      'Compound message detected — executing tasks sequentially',
    );
